...
{
public:
    explicit NatNetClient( ConnectionType connectionType = ConnectionType_Multicast );
    ~NatNetClient();

    ErrorCode Connect( const sNatNetClientConnectParams& pConnectParams );
    ...
    NATNET_DEPRECATED( "The constructor taking an int argument is deprecated; use the ConnectionType enumeration values instead." )
    ...
    ErrorCode Initialize( const char* szLocalAddress, const char* szServerAddress, int HostCommandPort = 0, int HostDataPort = 0 )
    {
        ...
        connectParams.serverCommandPort = HostCommandPort;
        connectParams.serverDataPort = HostDataPort;
        connectParams.serverAddress = szServerAddress;
        connectParams.localAddress = szLocalAddress;
